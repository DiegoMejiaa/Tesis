...
public abstract class BluetoothConnection
{
    ...
    public void connect(String address, UUID uuid) throws IOException {
        ...
        // Conexión con FALLBACKS. El socket "seguro" suele fallar con ESP32/HC-05
        // con "read failed, socket might closed or timeout, read ret: -1". Si pasa,
        // se intenta el socket inseguro y, por último, el método reflexivo
        // createRfcommSocket(canal 1), que resuelve la gran mayoría de los casos.
        try {
            socket.connect();
        } catch (IOException errorSeguro) {
            try { socket.close(); } catch (IOException ignored) {}
            try {
                socket = device.createInsecureRfcommSocketToServiceRecord(uuid);
                socket.connect();
            } catch (IOException errorInseguro) {
                try { socket.close(); } catch (IOException ignored) {}
                try {
                    ...
                    socket.connect();
                } catch (Exception errorReflexion) {
                    throw new IOException(
                            "No se pudo abrir el socket RFCOMM (seguro, inseguro y "
                                    + "canal 1 fallaron): " + errorSeguro.getMessage());
                }
            }
        }
